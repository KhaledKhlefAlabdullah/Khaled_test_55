...
* fix: `FullyQualifiedStrictTypesFixer` - do not change case of the symbol when there's name collision between imported
  class and imported function (#7750)
...
* feat: Add `union_types` option to `phpdoc_to_param_type`, `phpdoc_to_property_type`, and `phpdoc_to_return_type`
  fixers (#7672)
...
* fix: Several improvements for `fully_qualified_strict_types` (respect declared symbols, relative imports, leading
  backslash in global namespace) (#7679)
...
* fix: `fully_qualified_strict_types` with `leading_backslash_in_global_namespace` enabled - handle reserved types in
  phpDoc (#7648)
...
* refactor: Tokens - fast check for non-block in 'detectBlockType', evaluate definitions only once in '
  getBlockEdgeDefinitions' (#7655)
...
* bug: BlankLineBeforeStatementFixer - do not enforce/add a blank line when there is a blank line between the comment
  and the statement already (#7190)
...
* bug: StatementIndentationFixer - fix for multiline arguments starting with "new" keyword preceded by class
  instantiation (#6914)
...
* bug #6170 NonPrintableCharacterFixer - fix for string in single quotes, having non-breaking space, linebreak, and
  single quote inside (kubawerlos)
...
* minor #6011 NoSuperfluousPhpdocTagsFixer - Remove superfluous annotation `@abstract` and `@final` (liquid207,
  SpacePossum)
...
* minor #3384 diff-format option: drop sbd diff, use udiffer by default, drop SebastianBergmannDiffer and
  SebastianBergmannShortDiffer classes (keradus)
...
* minor #3390 Drop deprecated rules: blank_line_before_return, hash_to_slash_comment, method_separation,
  no_extra_consecutive_blank_lines, no_multiline_whitespace_before_semicolons and pre_increment (keradus)
...
* minor #4666 Remove deprecated rules: lowercase_constants, php_unit_ordered_covers, silenced_deprecation_error (
  keradus)
...
* bug #5362 SingleSpaceAfterConstructFixer: Do not adjust whitespace before multiple multi-line extends (localheinz,
  SpacePossum)
...
* minor #5326 DX: relax composer requirements to not block installation under PHP v8, support for PHP v8 is not yet
  ready (keradus)
...
* bug #5032 SingleLineAfterImportsFixer - fix for line after import (and before another import) already added using
  CRLF (kubawerlos)
...
* bug #5045 BacktickToShellExecFixer - add priority relation to NativeFunctionInvocationFixer and SingleQuoteFixer (
  kubawerlos)
...
* minor #5050 DX: moving integration test for braces, indentation_type and no_break_comment into right place (
  kubawerlos)
...
* bug #4919 PhpUnitTestAnnotationFixer - fix function starting with "test" and having lowercase letter after (
  kubawerlos)
...
* bug #4626 FinalPublicMethodForAbstractClassFixer - Do not attempt to mark abstract public methods as final (
  localheinz)
...
* minor #4524 MethodArgumentSpaceFixerTest - make explicit configuration to prevent fail on configuration change (
  keradus)
...
* bug #4384 MethodArgumentSpaceFixer - fix for on_multiline:ensure_fully_multiline with trailing comma in function
  call (kubawerlos)
...
* bug #4303 FullyQualifiedStrictTypesFixer - Fix the short type detection when a question mark (nullable) is prefixing
  it. (drupol)
* bug #4313 SelfAccessorFixer - fix for part qualified class name (kubawerlos, SpacePossum)
* bug #4314 PhpUnitTestCaseStaticMethodCallsFixer - fix for having property with name as method to update (kubawerlos,
  SpacePossum)
...
* minor #4306 DX: Drop HHVM conflict on Composer level to help Composer with HHVM compatibility, we still prevent HHVM
  on runtime (keradus)
...
* minor #4295 DX: MethodArgumentSpaceFixerTest - fix edge case for handling different line ending when only expected
  code is provided (keradus)
...
* bug #4009 TypeAlternationTransformer - Fix pipes in function call with constants being classified incorrectly (ntzm,
  SpacePossum)
* bug #4022 NoUnsetOnPropertyFixer - refactor and bugfixes (kubawerlos)
* bug #4036 ExplicitStringVariableFixer - fixes for backticks and for 2 variables next to each other (kubawerlos,
  Slamdunk)
...
* bug #4071 DX: do not insert Token when calling removeLeadingWhitespace/removeTrailingWhitespace from Tokens (
  kubawerlos)
...
* bug #4100 PhpdocAddMissingParamAnnotationFixer - Handle variable number of arguments and pass by reference cases (
  SpacePossum)
...
* minor #3774 PhpUnitTestClassRequiresCoversFixer - Remove unneeded loop and use phpunit indicator class (BackEndTea,
  SpacePossum)
...
* bug #3626 ArrayIndentationFixer: priority bug with BinaryOperatorSpacesFixer and MethodChainingIndentationFixer (
  Slamdunk)
...
* minor #3539 Update UnifiedDiffOutputBuilder from gecko-packages/gecko-diff-output-builder usage after it was
  incorporated into sebastian/diff (keradus)
...
* bug #3265 YodaFixer - fix problems of block statements followed by ternary statements (weareoutman, keradus,
  SpacePossum)
...
* bug #3350 PhpUnitNoExpectationAnnotationFixer - fix handling of multiline expectedExceptionMessage annotation (
  Slamdunk)
...
* bug #3420 SingleLineCommentStyleFixer - fix 'strpos() expects parameter 1 to be string, boolean given' (keradus,
  SpacePossum)
...
* bug #3244 PhpUnitNoExpectationAnnotationFixer - expectation extracted from annotation shall be separated from rest of
  code with one blank line (keradus)
...
* bug #3062 BraceClassInstantiationTransformer - Fix instantiation inside method call braces case (julienfalque,
  keradus)
...
* feature #2684 BracesFixer - new options for braces position after control structures and anonymous constructs (
  aidantwoods, keradus)
* feature #2701 NoExtraConsecutiveBlankLinesFixer - Add more configuration options related to switch statements (
  SpacePossum)
...
* bug #2686 Commands readme and describe - fix rare casing when not displaying some possible options of configuration (
  keradus)
...
* minor #2483 DX: AbstractIntegrationTestCase does not use IntegrationCase::shouldCheckPriority, logic is now automated
  and method is now deprecated (keradus)
...
* bug #2408 SingleClassElementPerStatementFixer, PhpdocSeparationFixer - add missing WhitespacesAwareFixerInterface
  interface (keradus)
...
* feature #1819 Added DirConstantFixer, ModernizeTypesCastingFixer, RandomApiMigrationFixer (kalessil, SpacePossum,
  keradus)
...
* minor #1762 Remove ConfigInterface::getDir, ConfigInterface::setDir, Finder::setDir and whole FinderInterface (
  keradus)
...
* minor #1821 Ensure that PhpUnitDedicateAssertFixer runs after NoAliasFunctionsFixer, clean up NoEmptyCommentFixer (
  SpacePossum)
...
* bug #2051 NativeFunctionCasingFixer - call to constructor with default NS of class with name matching native function
  name fix (SpacePossum)
...
* minor #1663 DuplicateSemicolonFixer - Remove duplicate semicolons even if there are comments between those (
  SpacePossum)
...
* bug #2092 ConcatWithoutSpacesFixer, OperatorsSpacesFixer - fix too many spaces, fix incorrect fixing of lines with
  comments (SpacePossum)
...
* bug #1576 SpacesBeforeSemicolonFixer - do not remove space before semicolon if that space is after a semicolon (
  SpacePossum)
...
* bug #1048 MultilineArrayTrailingCommaFixer, SingleArrayNoTrailingCommaFixer - using heredoc inside array not causing
  to treat it as multiline array (keradus)
...
* minor #934 StdinFileInfo::getFilename - Replace phpdoc with normal comment and add back empty line before return (
  ceeram)
...
* bug #976 PhpdocIndentFixer - fix error when there is a comment between docblock and next meaningful token (keradus,
  ceeram)
...
* bug #908 BracesFixer - fix invalid inserting brace for control structure without brace and lambda inside of it (
  keradus)
